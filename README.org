...
* Variables & Arithmetic
  ...
  Arithmetic operators are functions with 3 arguments, *+ a b c* is *c
  = a + b*.  Available operators: +, -, *, /, % (module), MIN, MAX,
  POW (A**B), ROOT (A**(1/B))
...
* Flow Control
  ...
  *Blocks* begin with [ and end with ] and can hold any amount of
  ...
  invoked _count_ times.

* Animation (best part)

  You can use command *WAIT ms* to pause drawing for a moment. Look in
  the [[Examples/animation.logo]] and [[Examples/gradients.logo]] for
  animation examples.
